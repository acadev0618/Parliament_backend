...
class BudgetInformationController extends Controller {

    public function index() {
        if(Session::get('display_name')) {

            $data = DB::select('select * from downloads where kind = 5', [1]);

            return view('budgetInformation')->with(
            	['data' => $data, 
            	'sliderAction' => 'budget', 
            	'subAction' => '']
            );
    ...
    public function createBudgetInformation(Request $request) {
    ...
    public function updateBudgetInformation(Request $request) {
    ...
    public function deleteBudgetInformation(Request $request) {
        $id = array(
            'id' => $request->id
        );
    ...
    public function multiDeleteBudgetInformation(Request $request) {
        $ids = $request->ids;
        $ids = explode(',', $ids);
        $result = 0;
