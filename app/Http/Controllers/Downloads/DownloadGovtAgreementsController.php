...
class DownloadGovtAgreementsController extends Controller {

    public function index() {
        if(Session::get('display_name')) {

            $data = DB::select('select * from downloads where kind = 1', [1]);

            return view('downloads.govtAgreements')->with(
            	['data' => $data, 
    ...
    public function createGovtAgreements(Request $request) {
    ...
    public function updateGovtAgreements(Request $request) {
    ...
    public function deleteGovtAgreements(Request $request) {
        $id = array(
            'id' => $request->id
        );
    ...
    public function multiDeleteGovtAgreements(Request $request) {
        $ids = $request->ids;
        $ids = explode(',', $ids);
        $result = 0;
